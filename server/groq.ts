...
export async function analyzeResumeWithGroq(resumeText: string, jobs: Job[], priorDocument?: ResumeDocument | null) {
  ...
  const ai = await aiJson({ name: "carrerfit_resume_analysis", system, user, schema: analysisSchema, maxTokens: 7600, model: process.env.OPENAI_RESUME_MODEL, reasoningEffort: "high" });
  if (ai) {
